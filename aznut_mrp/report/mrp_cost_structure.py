        ...
        for product in productions.mapped('product_id'):
            ...
            if Workorders:
                query_str = """SELECT
                                ...
                                GROUP BY wo.production_id, wo.id, op.id, wo.name, wc.costs_hour, wc.production_station, currency_table.rate
                                ORDER BY wo.name
                            """.format(currency_table=currency_table, )
                self.env.cr.execute(query_str, (tuple(Workorders.ids),))
                for mo_id, dummy_wo_id, op_id, wo_name, duration, cost_hour, currency_rate in self.env.cr.fetchall():
                    cost = duration / 60.0 * cost_hour * currency_rate
                    total_cost_by_mo[mo_id] += cost
                    operation_cost_by_mo[mo_id] += cost
                    operations.append([op_id, wo_name, duration / 60.0, cost_hour * currency_rate])
